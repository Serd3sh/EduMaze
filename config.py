...
# Проверок "на дурака" НЕТ. Предполагается, что раз у человека есть исходный код, то он сам понимает что делает
# В релизе такого изменения свойств не будет
...
# Это связанно из-за технических особенностей и скорости выполнения кода

# (***) - Обязательно нечетная цифра(-ы) из-за связанных со свойством особенностей
...
IMAGES = {
    ...
    # так же можно переопределить размер текстуры. Делать это не желательно, тк не везде получилось это реализовать
    # пол умолчанию используется DEFAULT_CELL_SIZE

    # Пример:
    # [0, "pic1.png"]  - [0, 10). В промежутке 10 чисел = шанс 10%
    # [10, "pic2.png"] - [10, 40). Аналогично. Шанс = 40-10 = 30%
    # [40, "pic3.png"] - [40, 85). Шанс = 85-40 = 45%
    # [85, "pic4.png"] - [85, 99). Шанс = 99-85 = 14%
    # [99, "pic5.png"] - [99, 100). Шанс = 100-99 = 1%

    # элементы размещать СТРОГО в возрастающем порядке по числам
    # обязательно нумеровать с 0
    ...
    "teacher": {
        ...
        # хоть размер и можно переопределить, изменять его не рекомендуется, тк таковое еще не тестировалось
        # со значениями по умолчанию проблем нет
...
QUESTIONS = {
    ...
    # Длинна вопроса может быть любой, текст вопроса может переноситься на новую строку
    # Ответы к вопросам должны быть короткими, они не могут переноситься на новую строку
